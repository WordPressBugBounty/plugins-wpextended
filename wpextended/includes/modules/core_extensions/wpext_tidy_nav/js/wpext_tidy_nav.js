...
jQuery('select#by_user_role').on('change', function(e) {
 ...
 if(admin_user_role != null || admin_user_data != ''){
   ...
   jQuery('.wpext-loader-container').css("display", "block");
   jQuery('#wp-extended-tidymenu-frm').css("opacity", "0");
       ...
       jQuery.ajax({
        ...
        data: {
          ...
          success: function (data) {
            jQuery('.wpext-loader-container').css("display", "none");
            jQuery('#wp-extended-tidymenu-frm').css("opacity", "1");
            ...
            if(admin_user_role == 'administrator'){
              jQuery('.update-role').addClass("d-none");              
            }else{
                jQuery('.update-role').removeClass("d-none").prop('disabled', false);
            }
...
jQuery(document).ready(function($) {
    ...
    function wpext_saveTableOrder_by_user_role() {
         ...
         var role_menu_order = [];
         var data_key = [];
        jQuery("body #userlist-of-sections #tbody .wpext_user_role").each(function() {
