...
$log_block_time = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT * FROM {$failed_record} WHERE status = %d ORDER BY id DESC",
        1
    )
);
            ...
            <tbody>
                ...
                foreach ($log_block_time as $value) { ?>
                    <tr class="wpext_export_posts block_list item-list" id="<?php echo esc_attr($value->id); ?>">
                        ...
                        <td class="module-name has-text-centered text-center">
                            <?php echo esc_html(absint($value->locktime)); ?>
                            <?php esc_html_e('minutes', WP_EXTENDED_TEXT_DOMAIN); ?>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
